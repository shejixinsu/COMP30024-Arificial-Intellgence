...
def main():
    with open(sys.argv[1]) as file:
        data = json.load(file)
    exits=find_exit(data["colour"])
    route=[]
    for piece in data["pieces"]:
        route+=iterative_deepening_search(piece,exits,data["blocks"])
...
def child_nodes(current,blocks,visited):
    ...
    for node in surrounding(current):
        if (node not in blocks) and (node not in visited) and in_board(node):
            nodes.append(node)
        elif (jump_to(current,node) not in blocks) and (jump_to(current,node) not in visited) and in_board(jump_to(current,node)):
